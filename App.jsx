...
export default function CleaningServiceSite() {
  ...
  const calculateQuote = () => {
    ...
    if (roomCount > 3) {
      adjustedPrice += (roomCount - 3) * 15;
    }

    if (squareFootage > 1200) {
      const extraSqFt = squareFootage - 1200;
      adjustedPrice += (extraSqFt / 100) * 5;
    }
    ...
    <div style={{ fontFamily: 'sans-serif', background: '#FAF8F6', color: '#1E3A5F', minHeight: '100vh' }}>
      ...
      <main style={{ padding: '2rem', maxWidth: '720px', margin: '0 auto' }}>
        ...
        <section style={{ background: '#FAF8F6', padding: '1.5rem', borderRadius: '0.75rem' }}>
          ...
          <ul>
            <li>Weekly - Save 20% - {quote ? `$${(parseFloat(quote.split('$')[1]) * 0.8).toFixed(2)}` : '$0.00'} per visit</li>
            <li>Biweekly - Save 10% - {quote ? `$${(parseFloat(quote.split('$')[1]) * 0.9).toFixed(2)}` : '$0.00'} per visit</li>
            <li>Monthly - Standard Rate - {quote ? `$${parseFloat(quote.split('$')[1]).toFixed(2)}` : '$0.00'} per visit</li>
          </ul>
        </section>
